...
import { fetchOsmSiteContext } from "@/lib/site-analysis/osm";
...
export async function GET(request: NextRequest) {
  try {
    ...
    const radius = asNumber(searchParams.get("radius")) ?? 500;
    ...
    const result = await fetchOsmSiteContext(lat, lng, radius);
